...
class LocaleManager implements LocaleManager.Interface {
  ...
  async init(req: Request) {
    if (this.session) {
      await this.session.initSession(req)
    } else {
      const XFW = req.headers.get('X-Forwarded-For')
      const sid = await getSid(req)
      this.session = new SessionManager({ sid, XFW })
      await this.session.initSession(req)
    }
  }
  ...
  async getUserLanguage(): Promise<string> {
    if (!this.session) throw new Error('SessionManager instance not initialized!')
