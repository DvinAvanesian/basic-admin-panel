...
class SessionManager implements SessionManager.Interface {
  ...
  private async initializeLogger(): Promise<void> {
    ...
    try {
      ...
      this.XFW = reqHeaders.get('X-Forwarded-For')
  ...
  async setAuthLevel(level: 'priv' | 'access'): Promise<void> {
    await this.initializeLogger()
    try {
      const auth = await this.getAuthStatus()
  ...
  async getAuthStatus(_options?: { errorIfNotFound?: true }): Promise<SessionManager.AuthStatus>
  async getAuthStatus(options?: SessionManager.GetPropOptions): Promise<SessionManager.AuthStatus | null> {
    await this.initializeLogger()
    try {
      ...
      if (auth) {
        ...
        if (auth.level === 'priv' && this.XFW !== auth.client) {
          ...
          await this.setAuthLevel('access')
  ...
  async getCurrentUser(): Promise<User.Fields<true>> {
    await this.initializeLogger()
    try {
